...
export const versions = {
	...
	experimental: ['1.13-pre7', '1.14-pre5', '1.14.3-pre4', '1.18-pre5', '1.18-pre8', '1.18-rc3']
};
...
export function isAvailable(version) {
	let included =
		...
		(experimental && versions.experimental.includes(version));

	if ((core.getInput('use-similar-versions').toLowerCase() ?? 'true') === 'true')
		included =
			included ||
			versions.versions.includes(similar(version)) ||
			(experimental && versions.experimental.includes(similar(version)));
