...
export function loadServer(callback) {
	...
	switch (runtime['download'] ?? 'url') {
		case 'buildtools': {
			...
			const flags = runtime['flags'].replaceAll('{version}', versions.current);
		...
		case 'url': {
			...
			switch (current) {
				...
				case 'fabric': {
					if (!build)
						throw new TypeError(
							"Fabric loaders require 'build' parameter to be set (ex: '0.11.5/1.0.1')"
						);
					break;
				}
				default: {
					throw new TypeError(
						`Download method ${runtime['download']} not found: Could not find {build} for ${current}`
					);
				}
			}
		...
		default: {
			throw new TypeError(
				`Download method ${runtime['download']} not found: Invalid Runtime ${current}`
			);
		}
	}
}
